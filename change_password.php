...
if(isset($_POST["old"])){
        ...
        $new = getValue($_POST["new"]);
            ...
            $sql =  "SELECT * FROM users WHERE employee_id=". $_SESSION["login"] . " AND user_password= PASSWORD('". $old ."')";
        $result = mysqli_query($con, $sql);
    if(mysqli_num_rows($result) == 1){
        $sql = "UPDATE users SET user_password= PASSWORD('$new') WHERE employee_id=". $_SESSION["login"];
...
<div class="card card-body">
    <form method="post" id="change" >
        <div class="input-group-text m-2">
            <span class="w-25 h6 mr-5">Old Password:</span> 
            <input autofocus="autofocus" type="password" class="form-control " name="old">
        </div>
        <div class="input-group-text m-2">
            <span class="w-25 h6 mr-5">New Password:</span>
            <input minlength="6" id="new" type="password" class="form-control" name="new">
        </div>
        <div class="input-group-text m-2">
            <span class="w-25 h6 mr-4">Retype Password:</span> 
            <input minlength="6" id="retype" type="password" class="form-control" name="retype">
        </div>
